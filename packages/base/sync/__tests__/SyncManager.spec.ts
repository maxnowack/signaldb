...
it('should clear all internal data structures on dispose', async () => {
  ...
  // @ts-expect-error - private property
  await expect(() => syncManager.changes.insert({})).rejects.toThrow('Collection is disposed')
  // @ts-expect-error - private property
  await expect(() => syncManager.snapshots.insert({})).rejects.toThrow('Collection is disposed')
  // @ts-expect-error - private property
  await expect(() => syncManager.syncOperations.insert({})).rejects.toThrow('Collection is disposed')
})
